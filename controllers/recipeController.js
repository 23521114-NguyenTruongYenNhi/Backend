...
export const getRecipes = async (req, res) => {
  try {
    // UPDATE: Only fetch recipes with status 'approved'
...
export const searchRecipes = async (req, res) => {
  try {
    ...
    // UPDATE: Initialize query with status 'approved'
...
export const createRecipe = async (req, res) => {
  try {
    const recipe = new Recipe({
      ...req.body,
      author: req.user._id,
      // Note: status will default to 'pending' based on Schema definition
    });

    const createdRecipe = await recipe.save();

    // Add to user's created recipes list
